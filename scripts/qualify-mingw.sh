...
configure_and_test()
{
    ...
    "$script_dir/repeat-ctest.sh" "$build_dir" "$repeat_count" \
        'wepoll_ex_windows_(api|stress|backpressure|compat_concurrent_ctl|pwait2_(conversion|generation_deadline|readiness_wins|close|fallback)|pipe_.*|waitable_(zero_.*|timer_et|queued_rearm)|state_waitable_(zero_(callback|ready)|queued_rearm)|fault_waitable_(ready_node_alloc|zero_disarm)|socket_events_(aliases|oob_(lt|et|oneshot|mod|inline_(lt|et)))|events_(mapping|status))$'
}
