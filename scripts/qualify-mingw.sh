...
large_afd_index=${WEPOLL_EX_LARGE_AFD_INDEX:-OFF}
...
configure_and_test()
{
    ...
    cmake -S "$repo_dir" -B "$build_dir" -G "MinGW Makefiles" \
        ...
        -DWEPOLL_EX_LARGE_AFD_INDEX="$large_afd_index" \
    ...
    "$script_dir/repeat-ctest.sh" "$build_dir" "$repeat_count" \
        'wepoll_ex_windows_(api|stress|backpressure|iocp_batch|cancel_failure|del_cancel_failure|compat_(concurrent_ctl|del_add|mod_before_wait|two_epfds|idle_two_epfds)|pwait2_(conversion|generation_deadline|readiness_wins|close|fallback)|pipe_.*|waitable_(zero_.*|timer_et|queued_rearm)|edge_(same_epfd_et|exclusive_mixed|ordinary_(multi|prearmed)|explicit_(rearm|terminal|fin|oneshot|contract)|local_shutdown_.*)|state_(socket_add_(submit_failure|immediate_success)|aux_add_lazy|transitional_idle|exclusive_mixed_submit|explicit_(rearm|oneshot_rearm)_state|waitable_(zero_(callback|ready)|queued_rearm)|udp_readless_(error|park|rollback)|large_wait|pending_expansion_ready_race|tcp_current_(rdhup|reset|et_snapshot))|fault_(afd_(groups|submit|submit_batch|refresh_submit|key_(fallback|order))|wake_post|iocp_post_immediate_socket|error_info_native|shutdown_(ready_node_alloc|cancel_retry)|waitable_(ready_node_alloc|zero_disarm))|socket_events_(aliases|inert_bits|round_robin|duplicate_registration|prewait_(refresh|scale)|oob_(lt|et|oneshot|mod|inline_(lt|et))|udp_readless_data|udp_error_v6_(zero|err|out_et|readless_(et|oneshot)))|events_(mapping|status))$'
}
